...
const LessonPage = () => {
  ...
  const checkEnrolled = async (checkId) => {
    const { data: response } = await getApi(
      dispatch,
      `${GET_COURSE}/${checkId}`
    );
    ...
    if (response?.status) {
      if (
        response?.data?.enrolledUser.length === 0 ||
        !response?.data.enrolledUser[0].enrolls.isEnroll
      ) {
        history.push(`/course/${checkId}`);
      }
      setIsEnrolled(true);
    }
  };
  ...
  const addLessonProgress = async () => {
    ...
    const { data: response } = await postApi(dispatch, ADD_LESSON_PROGRESS, {
      lessonId: id,
      startTime,
    });
  ...
  const nextPageHandler = async () => {
    try {
      ...
      if (updateLesson.success) {
        ...
        const startTime = moment().toDate().getTime().toString();
        const { data: addLesson } = await postApi(
          dispatch,
          ADD_LESSON_PROGRESS,
          {
            lessonId: next.id,
