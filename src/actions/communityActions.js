...
export const searchCommunities = (search) => async (dispatch) => {
  try {
    ...
    const { data } = await getApi(
      dispatch,
      `${process.env.REACT_APP_API_BASE_URL}/api/communities/search?name=${search}`
    );
    dispatch({
...
export const listUserCommunities =
  ({ userPageNumber = "" }) =>
  async (dispatch) => {
    try {
...
export const searchUserCommunities = (userId, search) => async (dispatch) => {
  try {
    ...
    const { data } = await getApi(
      dispatch,
      `${process.env.REACT_APP_API_BASE_URL}/api/communities/user/search?name=${search}`
    );
    dispatch({
...
export const createCommunity = (newCommunity) => async (dispatch) => {
  ...
  try {
    ...
    await postApi(
      dispatch,
      `${process.env.REACT_APP_API_BASE_URL}/api/communities/add`,
      formData
    );
    dispatch({
...
export const joinCommunity = (communityId) => async (dispatch) => {
  try {
    ...
    await postApi(
      dispatch,
      `${process.env.REACT_APP_API_BASE_URL}/api/communities-users/follow`,
      { communityId }
    );
