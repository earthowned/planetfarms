...
export const listQuestions = (lessonId) => async (dispatch) => {
  try {
  ...
  async (dispatch, getState) => {
    try {
      ...
      const data = await axios.delete(
        `${process.env.REACT_APP_API_BASE_URL}/api/questions/${deleteId}`
      );
      dispatch({
...
export const updateQuestion = (newQuestions) => async (dispatch) => {
  try {
    ...
    const { id, question, answer, options, testId, type, lessonId } =
      newQuestions;
    const data = await axios.put(
      `${process.env.REACT_APP_API_BASE_URL}/api/questions/${id}`,
      {
        question,
        answer,
        options,
