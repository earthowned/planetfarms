...
export const listCalendarEvents = () => async (dispatch) => {
  try {
...
export const createCalendarEvent = (newCalendarEvent) => async (dispatch) => {
  try {
    dispatch({
    ...
    const { data } = await postApi(
      dispatch,
      `${process.env.REACT_APP_API_BASE_URL}/api/calendar/add`,
      newCalendarEvent
    );
    dispatch({
