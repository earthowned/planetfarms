...
const Pagination = ({ pageNumber = 1, resourceList, setPageNumber }) => {
  ...
  useEffect(() => {
    const addedPages = [...Array(resourceList?.totalPages).keys()].map(
      (num) => num + 1
    );
    const offset = 3 - pageNumber > 0 ? 3 - pageNumber : 0;
    let visiblePages = addedPages.slice(
      pageNumber - 3 + offset,
      pageNumber + 2
    );
    if (pageNumber > 3) visiblePages = [1, ...visiblePages];
    if (addedPages.length - pageNumber > 2)
      visiblePages.push(resourceList?.totalPages);
  ...
  return (
    <div className="pagination-wrapper">
      <div className="pagination">
        ...
        <button
          type="button"
          disabled={pageNumber >= resourceList?.totalPages}
          onClick={() => handlePagination(1, +1)}
        >
          Next
        </button>
